...
export default async function handler(req, res) {
  ...
  // ── REGISTER ──
  if (action === 'register') {
    // Require admin secret to create users (prevents random signups)
    ...
    await redis.set(key, { username: username.toLowerCase(), passwordHash: hashPwd(password), createdAt: Date.now() });
    // Add to users list
    await redis.sadd('users', username.toLowerCase());

    const token = signToken({ username: username.toLowerCase() });
    return res.status(200).json({ token, username: username.toLowerCase() });
  }
